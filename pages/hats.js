  ...
  <div className="container px-5 py-10 mx-auto">
    <div className="flex flex-wrap -m-4 ">
      ...
      <div className="lg:w-1/4 md:w-1/2 p-4 w-full cursor-pointer shadow-lg">
        ...
        <div className="mt-4">
          <h3 className="text-gray-500 text-xs tracking-widest title-font mb-1">Hats</h3>
...
export async function getServerSideProps(context) {
  ...
  let products =await Product.find({category:"hats"})
  let hat ={};
  for(let item of products){
    if(item.title in hat){
      if(!hat[item.title].color.includes(item.color) && item.availableQty>0){
        hat[item.title].color.push(item.color)
      }
      if(!hat[item.title].size.includes(item.size) && item.availableQty>0){
        hat[item.title].size.push(item.size)
      }
    }
    else{
      hat[item.title]=JSON.parse(JSON.stringify(item))
      if(item.availableQty>0){
        hat[item.title].color=[item.color]
        hat[item.title].size=[item.size]
      }
    }
  }
  return {
    props: {products: JSON.parse(JSON.stringify(hat))} // will be passed to the page component as props
  }
}
